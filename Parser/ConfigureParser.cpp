...
ConfigParseForCodeAnal::~ConfigParseForCodeAnal()
{
  ...
  delete pHandleNamespaceDefinition;
...
bool ConfigParseForCodeAnal::Attach(const std::string& name, bool isFile)
{
  if(pToker == 0)
    return false;
  if (pIn != nullptr)
    pIn->close(); // release previous file
...
void ConfigParseForCodeAnal::ConfigFuncDef()
{
...
void ConfigParseForCodeAnal::ConfigDeclaration()
{
