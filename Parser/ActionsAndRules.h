...
/*
  ...
  ====================
  ver 3.4 : 13 Mar 2017
  - fixed bug in lambda detection
  - add some details to declaration analysis
  ver 3.3 : 26 Feb 2017
...
{  
  ...
  public:
    ...
    TypeTable& getTypeTable() { return tt; }
  ...
  public:
    ...
    void doActionOperator(ASTNode* top, const Scanner::ITokCollection* pTc, size_t nameIndex)
    {
      size_t operIndex = pTc->find("operator");
      if (operIndex < pTc->length())
      {
        std::string name = "operator" + (*pTc)[operIndex + 1];// +(*pTc)[operIndex + 2];
        if ((*pTc)[operIndex + 2] != "(")
    ...
    void doActionMember(ASTNode* top, const Scanner::ITokCollection* pTc, size_t nameIndex)
    {
    ...
    void doActionLambda(ASTNode* top, const Scanner::ITokCollection* pTc, size_t nameIndex)
    {
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      doActionOperator(top, pTc, nameIndex);
      
      // is function a member of a class or struct?
      doActionMember(top, pTc, nameIndex);
      
      // is this a lambda?
      doActionLambda(top, pTc, nameIndex);

      // is this a std::function?
      size_t funcIdx = pTc->find("function");
      if (funcIdx == 0 || funcIdx == 2)  //function or std::function
      {
        size_t eqlIdx = pTc->find("=");
        if (eqlIdx < pTc->length())
          top->name_ = (*pTc)[eqlIdx - 1];
        else
          top->name_ = (*pTc)[pTc->length() - 2];
      }
    }
  };
  ...
  */
  ...
  {
  public:
    void doAccess(Repository* pRepo, const Scanner::ITokCollection* pTc)
    {
  ...
  public:
    ...
    bool isUsing(Scanner::SemiExp& se, const Scanner::ITokCollection* pTc, ASTNode* pCurrNode, DeclarationNode& declNode)
    {
      if (se[0] == "using")
      {
        if (se[1] == "namespace")
        {
          ...
          GrammarHelper::showParse("using namespace declar", *pTc);
        }
        else if (se.find("=") < se.length())
        {
          declNode.declType_ = DeclType::aliasDecl;
          declNode.declName_ = se[se.find("=") - 1];
          pCurrNode->decl_.push_back(declNode);
          GrammarHelper::showParse("alias declar", *pTc);
        }
        return true;
      }
      return false;
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (se[0] == "typedef")
      {
        ...
        if (se[se.length() - 2] == ")")
          declNode.declName_ = se[se.find(")") - 1]; // function pointer
        else
          declNode.declName_ = se[se.length() - 2];
        pCurrNode->decl_.push_back(declNode);
        GrammarHelper::showParse("typedef declar", *pTc);
        return;
      }
      
      if (isUsing(se, pTc, pCurrNode, declNode)) return;
