...
//----< get relative path from fileSpec >------------------------------

std::string Path::getRelativeFromPathToFile(const std::string& pathFrom, const std::string& fileTo)
{
  const size_t BufSize = 256;
  char buffer[BufSize];

  PathRelativePathToA(buffer, pathFrom.c_str(), FILE_ATTRIBUTE_DIRECTORY, fileTo.c_str(), FILE_ATTRIBUTE_NORMAL);

  return std::string(buffer);
}
//----< get relative path from fileSpec >------------------------------

std::string Path::getRelativeFromFileToFile(const std::string& fileFrom, const std::string& fileTo)
{
  const size_t BufSize = 256;
  char buffer[BufSize];

  PathRelativePathToA(buffer,fileFrom.c_str(), FILE_ATTRIBUTE_NORMAL, fileTo.c_str(), FILE_ATTRIBUTE_NORMAL);

  return std::string(buffer);
}
//----< get absolute path from fileSpec >------------------------------

std::string Path::getAbsoluteFileSpec(const std::string& fileSpec, const std::string& path)
{
  if (fileSpec.find(".\\") == 0)
    return path + fileSpec.substr(1);
