...
void showUsage()
{
  ...
  out << "\n  - 2st: path to subdirectory to store published html files. Default is {analPath}\\OutputHTML\\";
...
{
  std::ostringstream out;
  out << "\n     Analys Path: \"" << FileSystem::Path::getFullFileSpec(argv[1]) << "\"";

  int i = 2;
  out << "\n     Pubish Path: ";
  if (argv[2][0] != '/' && argv[2][0] != '*')
  {
    publishDir_ = FileSystem::Path::getFullFileSpec(argv[2]);
    i++;
  }
  else {
    publishDir_ = FileSystem::Path::getFullFileSpec(argv[1]) + "\\OutputHTML";
  }
  out << "\"" << publishDir_ << "\"\n     Args: ";
